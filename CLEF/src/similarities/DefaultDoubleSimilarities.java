...
public class DefaultDoubleSimilarities extends AbstractSimilarities {
	 ...
	 */
	public DefaultDoubleSimilarities(MultivaluedAttribute attribute) throws Exception {
		super(attribute);
	...
	public Node getNodeOfValues(HashSet<String> values) {
		...
		if (!valueAsString.contains("-") && !valueAsString.equals("*")) {
			...
			for (int i = getSimilarityValues().size() -1 ; i >= 0 ; i--) {
				HashSet<String> valueSet = getSimilarityValues().get(i);
