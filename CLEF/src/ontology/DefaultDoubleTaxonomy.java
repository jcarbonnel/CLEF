...
public class DefaultDoubleTaxonomy extends AbstractTaxonomy {
	...
	/**
	 * Creates a new ontology for double values corresponding to the specified attribute.
	 * This constructor checks that the type of the attribute corresponds to double and throws an Exception if it is not.
	 * 
	 * @param attribute an attribute having double values.
	 * @throws Exception if the type of the specified attribute is not "Double Attribute".
	 */
	public DefaultDoubleTaxonomy(Attribute attribute) throws Exception {
		super(attribute);
		
		if (!attribute.getType().equals("Double Attribute")) {
			throw new Exception ("[Wrong ontology instance] Trying to compute an ontology of doubles the attribute \"" + attribute.getName() + "\" of type " + attribute.getType());
		}
	}
	...
	public Node getNodeOfValues(HashSet<String> values) {
		...
		if (!valueAsString.contains("-") && !valueAsString.equals("*")) {
			...
			for (int i = getOntologyValues().size() -1 ; i >= 0 ; i--) {
				HashSet<String> valueSet = getOntologyValues().get(i);
