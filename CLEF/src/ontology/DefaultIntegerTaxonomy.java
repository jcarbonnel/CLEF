...
public class DefaultIntegerTaxonomy extends AbstractTaxonomy {
	...
	/**
	 * Creates a new ontology for integer values corresponding to the specified attribute.
	 * This constructor checks that the type of the attribute corresponds to integers and throws an Exception if it is not.
	 * 
	 * @param attribute an attribute having integer values.
	 * @throws Exception if the type of the specified attribute is not "Integer Attribute".
	 */
	public DefaultIntegerTaxonomy(Attribute attribute) throws Exception {
		super(attribute);
		
		if (!attribute.getType().equals("Integer Attribute")) {
			throw new Exception ("[Wrong ontology instance] Trying to compute an ontology of integer for the attribute \"" + attribute.getName() + "\" of type " + attribute.getType());
		}
	...
	public void computeMissingOntologyValues() {
		...
		for (String value : distinctValues) {
			...
			ontologyValues.add(valueSet);
		}
		Collections.reverse(ontologyValues);
	}
