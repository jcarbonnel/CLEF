...
public abstract class AbstractACPosetExtractor {
	...
	/**
	 * Map of concepts and their number from the AC-poset.
	 */
	private TreeMap<Integer, String> conceptMap;
	...
	public TreeMap<Integer, String> getConceptMap() {
	...
	public void extractConcepts() {
		...
		try {
			...
			this.conceptMap = new TreeMap<Integer, String>();
            ...
            .map(c -> c.replaceAll("\\|", " "))
            .map(c -> c.replaceAll(" ];$", ""))
            .forEach(c -> conceptMap.put(
            		Integer.valueOf(c.substring(0, c.indexOf(" "))), 
            		c.substring(c.indexOf(" ") + 1, c.length())));
			System.out.println("## Number of concepts in AC-poset: "+ conceptMap.size());
	...
	public void extractSuborder() {
		...
		try {
			 ...
			 */
			int index = 0;
			for (int i = 0 ; i < this.conceptMap.size() ; i++) {
	...
	public void computeReverseCompleteSuborder() {
		...
		for (int conceptNumberInCS = 0 ; conceptNumberInCS < conceptMap.size() ; conceptNumberInCS++) {
			
			this.completeSuborder.add(new ArrayList<Integer>());
			
			for (int conceptNumberInS = 0 ; conceptNumberInS < conceptMap.size() ; conceptNumberInS++) {
		...
		for (int i = 0 ; i < conceptMap.size()  ; i++) {
	...
	public void computeCompleteSuborder() {
		...
		for (int i = conceptMap.size() - 1 ; i >= 0 ; i--) {
