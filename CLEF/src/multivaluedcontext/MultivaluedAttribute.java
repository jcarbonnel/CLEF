 ...
 * 
 * A MultivaluedAttribute has an associated {@link AbstractSimilarities} of values, i.e., a meet-semi lattice of values representing their similarities.
 * 
...
public class MultivaluedAttribute extends Characteristic {
	 ...
	 */
	private AbstractSimilarities taxonomy;
	...
	public MultivaluedContext scaling(MultivaluedContext pm) {
		...
		try {
			...
			if (this.getType().equals("Literal Attribute")) {
				this.taxonomy = new LiteralSimilarities(this);
			} else if (this.getType().equals("Double Attribute")) {
				this.taxonomy = new DefaultDoubleSimilarities(this);
			} else {
				this.taxonomy = new DefaultIntegerSimilarities(this);
			}
			
			taxonomy.computeMissingValues();
			...
			for (HashSet<String> ontologyValueSet : taxonomy.getSimilarityValues()) {
