...
public class MultivaluedContext {
	...
	private void initFromCSVfile(String contextName) {
		try {
			...
			BufferedReader br = new BufferedReader(new FileReader("data/0_clean_PCMs/" + contextName));
	...
	public void computeLattice() {
		...
		try {
			...
			if (Files.exists(Paths.get(PCM_dir + "FCA/AC-poset/result.xml"))) {
				Files.delete(Paths.get(PCM_dir + "FCA/AC-poset/result.xml"));
			}
			
			if (Files.exists(Paths.get(PCM_dir + "FCA/result.xml"))) {
				Files.delete(Paths.get(PCM_dir + "FCA/result.xml"));
			}
